...
> 
> 1. **前端交互的革新**：
>    - AI 从纯文本输出转向生成交互式前端应用（如 PPT、流程图）。
>    - 提出从 \\"chat-first\\" 到 \\"artifact-first\\" 的交互转变。
> 
> 2. **Tool Use & Agent 能力**：
>    - 通过合成数据激发预训练模型的工具调用能力。
>    - 简单 Agent 框架示例：循环执行任务→调用工具→反馈结果。
> 
> 3. **开源的意义**：
>    - 倒逼技术透明化，避免闭源服务的 \\"hack 粉饰\\"。
>    - 社区协作加速生态发展（如量化、部署优化）。
> 
> 4. **决心与争议**：
>    - 坚持不依赖投流，靠模型硬实力竞争。
>    - 激进聚焦基础算法（停更 K1，全力投入 K2）。
>    - 批评跟风 Agent 产品，强调模型能力才是上限。
> 
> 5. **未来展望**：
>    - 承认 K2 不足，迫切期待 K3。
>    - 文末补充：对行业保持敬意，部分观点偏激。
...
于是我趁着清明假期肝了一天，从 [aider](https://aider.chat/) 抄了 workflow 和 prompt 做了个 demo 出来，交互仍然是 ChatBot 的形式， 但当用户问 “介绍一下小米 Su7” 时，普通的 chatbot 会给出一段文字简介， 我这个 demo 会直接输出一份图文并茂、可以交互的 PPT 一样的网页出来， 用户还可以继续提要求修改，什么“背景改成黑色”，“再补充介绍一下 Su7 Ultra” 之类的。
